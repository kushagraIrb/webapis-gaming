...
class CoinFlipService {
  static async currentCoinFlipMatch() {
    try {
      const currentCoinFlipMatch = await coinFlipModel.fetchCurrentMatch();
      return currentCoinFlipMatch;
    } catch (error) {
    ...
    static async getUserPincode(userId) {
      try {
        const user = await coinFlipModel.fetchUserPincode(userId);
    ...
    static async getLiveMatches(userId) {
      try {
        const matches = await coinFlipModel.fetchLiveMatches();
        ...
        const userBets = await coinFlipModel.getUserBetIds(userId, matchIds);

        for (const match of upcomingMatches) {
          const encryptedId = await this.encryptId(match.id);
          await coinFlipModel.updateEncryptedId(match.id, encryptedId);
    ...
    //     try {
    //         return await coinFlipModel.fetchPastMatches();
    //     } catch (error) {
    ...
    static async getMatchDetails(encryptedMatchId) {
      try {
          const match = await coinFlipModel.fetchMatchById(encryptedMatchId);
          if (!match) {
            throw new Error("Match not found");
          }
  
          const teamOneUsers = await coinFlipModel.fetchUsersInTeam(match.id, match.team_one_id);
          const teamTwoUsers = await coinFlipModel.fetchUsersInTeam(match.id, match.team_two_id);
          const totalUsers = await coinFlipModel.fetchTotalUsersInMatch(match.id);
    ...
    static async getTossTypes() {
        try {
            return await coinFlipModel.fetchTossTypes();
        } catch (error) {
    ...
    static async getUserBets(userId, encryptedMatchId) {
        try {
            return await coinFlipModel.fetchUserBets(userId, encryptedMatchId);
        } catch (error) {
    ...
    static async checkWinnerAnnounced(matchId) {
      try {
        ...
        const matchDetails = await coinFlipModel.getMatchById(matchId);
    ...
    static async CalculateBetCancelCharge(debitAmount) {
      try {
        return await coinFlipModel.CalculateBetCancelCharge(debitAmount);
      } catch (error) {
    ...
    static async refundWallet(refundData) {
      try {
        ...
        const insertStatus = await coinFlipModel.insertRefundTransaction(refundData);
        return insertStatus;
      } catch (error) {
    ...
    static async cancelBet(betId) {
      try {
        const updateStatus = await coinFlipModel.updateBetStatus(betId);
        return updateStatus;
      } catch (error) {
    ...
    static async isMatchCancelled(matchId) {
      try {
        const match = await coinFlipModel.getMatchStatus(matchId);
    ...
    static async isBetCancelled(userId, bet_id) {
      try {
        const bet = await coinFlipModel.getBetStatus(userId, bet_id);
    ...
    //     const betExists = await coinFlipModel.userHasBetOnMatch(userId, matchId);
    //     return !!betExists;
    //   } catch (error) {
    ...
    static async checkAlreadyBet(userId, matchId) {
      try {
        ...
        const matchDetails = await coinFlipModel.getMatchTime(matchId);
        ...
        const betExists = await coinFlipModel.userHasBetOnMatch(userId, matchId, matchDateTime.format('YYYY-MM-DD HH:mm:ss'));
    ...
    static async placeBet(betData) {
        try {
          const newBet = await coinFlipModel.placeBet(betData);
          return newBet;
        } catch (error) {
    ...
    static async findUserById(user_id) {
      try {
        const user = await coinFlipModel.findUserById(user_id);
        return user;
    ...
    static async cancelLiveBet(betData) {
      try {
        const newBet = await coinFlipModel.cancelLiveBet(betData);
        return newBet;
      } catch (error) {
        throw new Error('Error placing the bet');
      }
    ...
    static async getExtraTimeList() {
      try {
        const matches = await coinFlipModel.fetchExtraTimeLiveMatches();

...
class CoinFlipModel {
    ...
    static async getEligibleMatch_old() {
        try {
            ...
            // Full query string for debugging
            ...
            const [rows] = await db.promise().query(query, [currentDate, currentTime]);
              ...
              // ✅ SAME SIMPLE LOG STYLE AS BEFORE
                // fs.appendFileSync(
                //     'model_logs.log',
    ...
    static async getEligibleMatch_new() {
        try {
            const istTime = moment().tz("Asia/Kolkata");
            const currentDate = istTime.format("YYYY-MM-DD");
            const minus6 = istTime.clone().subtract(6, 'seconds').format("HH:mm:ss");
            const plus6  = istTime.clone().add(6, 'seconds').format("HH:mm:ss");
            ...
            //     SELECT * FROM tbl_upcoming_match_coinflip
            //     WHERE status = 1
            //     AND match_date = '${currentDate}'
            //     AND match_time BETWEEN '${minus6}' AND '${plus6}'
            //     ORDER BY id DESC
            //     LIMIT 1
            // `);
    ...
    static async createNewGame(newGameData) {
        ...
        const checkQuery = `
            SELECT COUNT(*) as count
            FROM tbl_upcoming_match_coinflip
            WHERE match_date = ? AND match_time = ?
        `;
        const checkParams = [matchDate, matchTime];
    
        const [existingMatch] = await logQuery(
            'createNewGame-CHECK',
            checkQuery,
            checkParams,
            () => db.promise().query(checkQuery, checkParams)
        ...
        if (existingMatch[0].count > 0) return;
    ...
    static async getHighestBidder(matchId) {
        const query = `
            SELECT 
                ...
                su.is_active
            FROM tbl_coin_bet b
            LEFT JOIN tbl_coin_selected_users su
            ...
            ORDER BY
                -- Prefer selected users
                CASE WHEN su.user_id IS NOT NULL THEN 0 ELSE 1 END,
                -- Among selected users, lowest forced_loss_count first
                su.forced_loss_count ASC,
                -- If tie → random
                RAND()
            LIMIT 1
        `;
    ...
    static async getCoinFlipHistory(start = 0, perPage = 5, user_id) {
        try {
            const query = `
                SELECT 
                    m.match_title, m.match_date, m.match_time,
                    CASE 
    ...
    static async getCoinFlipHistoryCount(user_id) {
        try {
            const query = `SELECT COUNT(*) as total FROM tbl_coin_bet b JOIN tbl_upcoming_match_coinflip m ON b.match_id = m.id WHERE b.user_id = ? AND b.bet_date >= DATE_SUB(NOW(), INTERVAL 3 DAY)`;
